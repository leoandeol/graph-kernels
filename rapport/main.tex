...
\renewcommand{\putbib}{\bibliographystyle{ieeetr}\bibliography{references}}
...
\begin{table}
	\begin{center}
		\begin{tabular}{|c|c|c||c|c||c|}
			\hline
			& \multicolumn{2}{c||}{Labels} & \multicolumn{2}{c||}{$\mu(k)$} & Complexity\\
			\cline{2-6}
			& None & Finite Set & $\lambda^{k}$ & Any & \\
			\hline
			Sylv. Equation & $\times$\footnotemark\footnotetext{foo bar} & $\times$\footnote{Theoretically} & $\times$ & &\\
			Conjugate Gradient & $\times$ & $\times$ & $\times$ & &\\
			Fixed point & $\times$ & $\times$ & $\times$ & &\\
			Spec. Decomp.& $\times$ & & $\times$ & $\times$ & \\
			\hline
		\end{tabular}
	\end{center}
...
The Support Vector Machines algorithm is not implemented since it is not the focus of this project. Instead, the famous library \textbf{libsvm}\cite{cc2011libsvm} is used through the python library scikit-learn\cite{pedregosa2011scikit} because of its efficiency and renown. Moreover, there are some hyperparameters to optimize, namely the penalty coefficient on classification mistakes $C$. Since it is not the topic of this project it has not been optimized.\todo{Faire ça rapidement}
...
\begin{table}
	\begin{center}
		\begin{tabular}{|c||c|c||c|c|}
			...
			%%% nb of graphs
			\hline
			& Accuracy & Comp. time & Accuracy & Comp. time\\
			\hline 
			Sylv. Eq. & $0.182\pm 8.25e-3$ & 2'35" & & \\
			Conj. Gr. & 0.182 & 2h21" & & \\
			Fixed Pn. & 0.182 & 4h25'56" & & \\
			Spec. De. & 0.180 & 1h23'10" & & \\
			\hline
		\end{tabular}
	\end{center}
...
todo
\todo{Mettre liste des tables/figures ou pas ?}
